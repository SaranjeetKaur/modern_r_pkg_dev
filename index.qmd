...
📚 Resources: [usethis.r-lib.org](https://usethis.r-lib.org/)  @cr-usethis_hex
...
`devtools` - Essential Development Workflow 🔧  [@cr-devtools_hex]{zoom-to="3"}
...
📚 Resources: [devtools.r-lib.org](https://devtools.r-lib.org/)  @cr-devtools_hex
...
:::{.cr-section layout="sidebar-left"}
GitHub Actions with r-lib/actions - CI/CD Setup 🚀  @cr-actions
...
Before pushing to CRAN, you KNOW your package works everywhere. That ✅ on your commits means:  @cr-custom_workflow
...
📚 Resources: [github.com/r-lib/actions](https://github.com/r-lib/actions)  @cr-actions
...
:::{.cr-section layout="sidebar-left"}
Package Structure with `pkgdown` Site Generation 🌐  @cr-pkgdown_hex
...
📚 Resources: [pkgdown.r-lib.org](https://pkgdown.r-lib.org)  @cr-pkgdown_hex
...
::::{.cr-section layout="sidebar-right"}
Adding dependencies to your R package 🧵  @cr-usethis_dep
...
:::{.cr-section layout="sidebar-left"}
`roxygen2`: Advanced Tags and Cross-References 📝  @cr-ro2_hex

Documents using advanced `roxygen2` features, with markdown-style writing! 🎯

Cross-references 🔗  @cr-crossref
...
📚 Resources: [roxygen2.r-lib.org](https://roxygen2.r-lib.org)  @cr-ro2_hex
...
**Semantic versioning**: Version number communicates risk. 1.2.3 → 1.2.4 = safe. → 2.0.0 = read changelog.  @cr-semver
...
Basic benchmarking  @cr-basic_benchmark
...
Memory profiling  @cr-memory_profiling
...
**Performance question**: "Is approach A faster than B?" Guessing = wrong. Microbenchmarking = proof.

**Memory matters**: Track memory allocation. Sometimes "faster" uses 10x memory. `bench` shows the tradeoff.  @cr-memory
...
**Catch regressions**: Include benchmarks in tests. New code slower? Test fails. Maintain performance intentionally.

Resources: [bench.r-lib.org](https://bench.r-lib.org/)
...
:::{.cr-section layout="sidebar-left"}
`rhub` - Multi-Platform Testing (Thread)  @cr-rhub_hex

Test your package on multiple platforms before CRAN submission.

Run CRAN checks: Tests on Debian, Windows, Fedora. Same platforms CRAN uses. Catch platform-specific issues early.  @cr-rhub_cran
...
**Platform specific**: Target specific platforms where you expect issues.  @cr-rhub_platforms

:::{#cr-rhub_platforms}
```r
rhub::check_on_windows()
rhub::check_on_macos()
rhub::check(platform = "debian-clang-devel", "fedora-gcc-devel")
```
:::

**Before submission**: Run rhub + GitHub Actions. If both pass, CRAN will likely accept.

Resources: [r-hub.github.io/rhub](https://r-hub.github.io/rhub/)  @cr-rhub_hex
...
:::{.cr-section layout="sidebar-right"}
S3, S4, and S7 Object Systems

Create robust object-oriented interfaces with R's object systems.

S3 methods (simple & common)  @cr-s3_methods

:::{#cr-s3_methods}
```r
#' @export
print.my_class <- function(x, ...) {
  cat("My object with", length(x), "elements\n")
  invisible(x)
  }
```
:::

S4 classes (formal & strict)  @cr-s4_classes
...
S7 (modern & robust)  @cr-s7_classes
...
Resources: [rconsortium.github.io/S7](https://rconsortium.github.io/S7)
:::
...
Enhanced messages  @cr-cli_messages
...
:::{.cr-section layout="sidebar-right"}
`rlang` - Tidy Evaluation in Packages  @cr-rlang_hex

Handle user expressions safely in package functions.

Basic tidy evaluation  @cr-basic_tidy_eval
...
Resources: [rlang.r-lib.org](https://rlang.r-lib.org/)  @cr-rlang_hex
...
:::{.cr-section layout="sidebar-left"}
CRAN Submission Checklist and `cran-comments.md`

Final steps for successful CRAN submission.

Pre-submission checklist  @cr-cran_checklist
...
Create `cran-comments.md`  @cr-cran_comments
...
Template  @cr-cran_template

:::{#cr-cran_template}
```markdown
## Test environments
* local: R 4.3.0
* GitHub Actions: windows, macOS, ubuntu (R-devel, release, oldrel)
* rhub: fedora, debian
...
Start with the **release issue checklist**: Creates a GitHub issue with complete pre-submission checklist. Everything you need to remember, in one place. Check boxes as you go.  @cr-release_issue
...
Document Everything in `cran-comments.md`: Tell CRAN reviewers: test environments, check results, platform-specific notes. They read this first. Be thorough, honest, respectful.  @cr-cran_comments
...
Common rejection culprits: Prevent these through thorough testing!  @cr-rejection_culprits

:::{#cr-rejection_culprits}
```markdown
- Examples >5 seconds
- Typos/grammar in docs
- Missing input validation (crashes on edge cases)
- Platform-specific bugs
- Unnecessary package bloat
```
:::

Submit and be patient: Initial review takes 2-7 days typically. If accepted, 🎉! If issues found, respond promptly, fix kindly, resubmit.  @cr-submit_cran
...
:::{.cr-section layout="sidebar-right"}
[Full blog](https://drmowinckels.io/blog/2025/rpackage-dev-calendar/)

Further reading: [r-pkgs.org](https://r-pkgs.org), [usethis.r-lib.org](https://usethis.r-lib.org)
:::